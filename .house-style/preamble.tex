...
\setlength{\parskip}{0pt plus 1pt}  % 1pt stretch lets TeX pull an extra line onto pages
...
% Widow/orphan control (150 = strong discouragement, not absolute prohibition;
% 10000 was causing large white gaps at page bottoms)
\widowpenalty=150
\clubpenalty=150
...
\hyphenation{Huddleston Pullum Langacker Goldberg Chomsky Labov Haspelmath
  Bybee Croft Millikan Boyd Khalidi Saussure Jakobson Bloomfield
...
\hypersetup{
  ...
  pdftitle={*(Un)grammatical: How grammaticality works}
}
\urlstyle{same}
...
% Keep APA's DOI hyperlinks but print a shorter label on the page.
...
% Judgement markers (zero-argument, for judgment slots: \ea[*]{\mention{text}})
\newcommand{\ungram}{\textsuperscript{*}}
\newcommand{\marg}{\textsuperscript{?}}
\newcommand{\mmarg}{\textsuperscript{??}}
\newcommand{\odd}{\textsuperscript{\#}}
...
% Definiteness thread: recurring end-of-chapter sidebar (Chs 4–9)
% Bringhurst-style: reduced size, partial hairline rule, letterspaced label
\newenvironment{definitenessthread}{%
  \par\addvspace{1.5\baselineskip}%
  \noindent\rule{3em}{0.4pt}\par\nobreak
  \vspace{0.3\baselineskip}\nobreak
  \begingroup\small\noindent
  {\addfontfeatures{LetterSpace=12}\textsc{definiteness thread}}.\enspace\ignorespaces
}{%
  \par\endgroup\addvspace{1\baselineskip}%
}
...
% Four indices: Subject (concepts), Names (authors), Languages, Lexical (mentioned words)
\makeindex[name=subject, title=Subject Index, columns=2, intoc]
\makeindex[name=names, title=Name Index, columns=2, intoc]
\makeindex[name=languages, title=Languages Index, columns=2, intoc]
...
\newcommand{\ixg}[1]{#1\index[languages]{#1}}       % Languages: inline + tag
\newcommand{\ixl}[1]{\textit{#1}\index[lexical]{#1@\textit{#1}}} % Lexical: italicised + sorted correctly
\newcommand{\ixls}[2]{\textit{#2}\index[lexical]{#1@\textit{#2}}} % Lexical: explicit sort key + display
\newcommand{\ixsq}[1]{\index[subject]{#1}}          % Subject: tag only (quiet)
\newcommand{\ixnq}[1]{\index[names]{#1}}            % Names: tag only (quiet)
\newcommand{\ixgq}[1]{\index[languages]{#1}}        % Languages: tag only (quiet)
\newcommand{\ixlq}[1]{\index[lexical]{#1@\textit{#1}}} % Lexical: tag only (quiet), sorted correctly
\newcommand{\ixlqs}[2]{\index[lexical]{#1@\textit{#2}}} % Lexical: quiet tag with explicit sort/display
\newcommand{\ixlgq}[3]{\index[lexical]{#1@\textit{#2} (#3)}\index[languages]{#3!#1@\textit{#2}}} % Lexical + language co-index
\newcommand{\ixlgqs}[4]{\index[lexical]{#1@\textit{#2} (#3, #4)}\index[languages]{#3!#1@\textit{#2} (#4)}} % Lexical + language + gloss
