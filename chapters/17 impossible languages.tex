...
Languages given by god(s)
UG evolves in humans
humans evolve language

writers and folks imagine impossible languages
\begin{itemize}
    \item magical spells
    \item the langauges of god(s)
    \item Borges: language families of Tlön from Tlön, Uqbar, Orbis Tertius
\end{itemize}
Linguist \& philosophers
\begin{itemize}
    \item Logical grammar
    \item Bishop John Wilkins' An Essay towards a Real Character and a Philosophical Language, published in 1668. Wilkins divides the universe into classes which are then subdivided and further subdivided, to produce a tree structure. ``If you sit down and make a sincere attempt to use the language, you discover the really important flaw, not in \textit{his} language, but in the whole idea of a philosophical language: when you speak in concepts, it's too damn hard to say anything'' \citep[?]{Okrent2009}.
    \item Cultural–historical approaches
    \item Structuralism: a sociological–semiotic theory
    \item Saussure
    \item Functionalism: language as a tool for communication
    \item Formalism: language as a mathematical–semiotic system
    \item Chomsky hierarchy
    \item islands
    \item OVS (later shown to exist)
    \item Geoffrey Samson 1974 "To make some progress, we have to be able to say that there are some rel's which could not possibly occur as natural languages."
    \item Post-structuralism: language as a societal tool
\end{itemize}

"An impossible language is a non-recursive one." \citep[31]{Moro2016}

``Impossible languages'' are learnable, but they're learned differently. \citep[58]{Moro2016}

``The notion of “impossible language” in the sense adopted here was inspired by the formulation of grammar first proposed in Chomsky 1962 and refined in Chomsky 1965'' \citep[123]{Moro2016}
