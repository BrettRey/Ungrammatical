...
% TODO: light rewrite of opening paragraphs -- mild AI-tic vocabulary
...
% EDITORIAL-HPC: This is the right qualification. Preserve it in revision:
% dependency locality explains why a coupling may stabilize, while the detector
% responds to the stabilized convention and its licensed alternatives.

Occasionally, a highway interchange is set up with a contra-lateral exit. One such case is to be found on Ontario's Highway 427 southbound to Eglinton Avenue. As the highway crosses Eglinton, it jogs sideways, and Highway 401 happens to intersect at just this point, with the mess in Figure \ref{fig:427map} \citep{427map} being the result. But this left-side exit wasn't capricious or accidental; it's constructed this way because the usual rightwards design would have been more complex to execute.
...
We find similar adjustments for complexity in language. One is the so-called \textsc{heavy NP (Noun Phrase) shift}. A \enquote{heavy} NP is one with a lot of words in it: \textit{a book that I had found in a used-book store in Paris} is a heavy NP, \textit{a book} is light. And the shift is a shift away from the verb, to the right in English, reflecting a \enquote{short-before-long} preference.

In heavy NP shift, the general ordering tendency is suppressed. The result, again, is to minimize the dependency distance between heads. In (\ref{ex:book-dep-dist-long}), we can count the number of words intervening between \mention{gave} and the head of each of the dependents (\mention{book}, \mention{to}, and \mention{for}) to find the dependency distance.
...
The basic version in (a) has a total dependency distance of nine words. The (b) version is grammatical, but with the heavy noun phrase plugged into the basic sentence structure, the dependency distance more than doubles, from nine words to twenty. On the other hand, under the heavy NP shift, the dependency distance drops back to 10 words. Just as in the highway 427 exit example, there's a good reason to adjust the structure to accommodate an unusual situation.

But if we try to do the same thing with a \enquote{light} NP (\ref{ex:light-NP-shift}), there's no justification, and so the unusual structure fails. It's ungrammatical. The shift is only allowed when there's a good reason for it.
% EDITORIAL-HPC: "Allowed when there's a good reason" sounds teleological.
% Better HPC framing: heavy NP shift is an entrenched conditional pattern;
% heaviness licenses the shift because the community has stabilized that
% form-value tradeoff, not because each token earns permission on the fly.
...
\ea\label{ex:CNN}
    \ea[]{\textit{A convolutional neural network \uline{architecture} \uline{is} formed by a stack of distinct layers that transform the input volume into an output volume (e.g. holding the class scores) through a differentiable function.}\\\hfill[\textsc{passive}, 0]}
    \ex[]{\textit{A \uline{stack} of distinct layers that transform the input volume into an output volume (e.g. holding the class scores) through a differentiable function \uline{forms} a convolutional neural network architecture.}\hfill[\textsc{active}, 21]}\z
\z
...
One more example illustrates the point. Consider the difference between \textit{I threw out the trash} and \textit{I threw the trash out}. Both are fine, but as the \mention{trash} noun phrase becomes weighed down with more and more words in (\ref{ex:trash-out}), the pressure for the \mention{out}-first version builds. Notice the feeling of resolution going from (f) to (g).
