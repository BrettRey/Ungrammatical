...
The case of the missing \textit{whose} suggests several important lessons about language and linguistic analysis:

\begin{enumerate}
    \item Absence of evidence is not evidence of absence. Just because we can't easily find examples of a construction doesn't mean it doesn't exist.

    \item Even native speakers' intuitions can be misleading. Constructions that seem ungrammatical in isolation might be perfectly acceptable in the right context.
    
    \item What we perceive as ungrammatical may simply be an overlooked form-context mismatch, overlooked because we haven't though hard enough about what the right context is or why it should be so.

    \item Linguistic theories need to account for rare as well as common phenomena. A complete theory of language can't just explain the everyday stuff; it needs to handle the edge cases too.

    \item Language is always full of surprises. Even after centuries of study, we're still discovering new things about how language works.
\end{enumerate}

These restrictions on antecedents and their relationship to ellipsis and pronouns mirror broader limitations found in syntactic structures like filler-gap constructions. For example, Winckel et al. (2025) demonstrate that discourse-based constraints, particularly those affecting subject islands in cleft constructions, further limit the acceptability of long-distance dependencies. This reinforces the idea that factors beyond syntax, such as focus-background dynamics, can impose additional restrictions on grammaticality. We first noted this connection in Chapter 1, and here it provides further insight into the complexity of constraints on extraction."